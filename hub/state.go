...
// provider is everything the hub knows about one provider instance.
...
// providers. State mutations arrive from relay reader goroutines while the
// Fyne event loop reads snapshots, so access is guarded by an RWMutex.
...
// messages have already been filtered by parseMessage.
...
// HasCap reports whether the provider advertised a capability. PLAYPAUSE
// and INFO do not have separate capabilities.
func (v providerView) HasCap(cap string) bool {
...
// displayPosition returns the best available playback position: the last
// reported POS, advanced by elapsed wall-clock time while the provider is
// playing. Stale values are tolerated and never adjusted backwards.
